...
Class Action {
	...
	function save_fuel(){
		...
		$data = " fuel = '$fuel' ";
		if(!empty($_FILES['img']['tmp_name'])){
			$fname_fuel = strtotime(date("Y-m-d H:i"))."_".$_FILES['img']['name'];
			$move_fuel = move_uploaded_file($_FILES['img']['tmp_name'], '../assets/img/'.$fname);
			if($move_fuel){
				$data .=", upload_fuel = '$fname_fuel' ";
			}
		}
		if(empty($id)){
			$save = $this->db->query("INSERT INTO fueling_sheet set ".$data);
		}else{
			$save = $this->db->query("UPDATE fueling_sheet set ".$data." where id=".$id);
		}
		if($save)
			return 1;
	}
	function delete_fuel(){
		extract($_POST);
		$delete = $this->db->query("DELETE FROM fueling_sheet where id = ".$id);
		if($delete)
			return 1;
	}
	...
	function save_report(){
		...
		$data = " report = '$report' ";
		if(!empty($_FILES['img']['tmp_name'])){
			$fname_report = strtotime(date("Y-m-d H:i"))."_".$_FILES['img']['name'];
			$move_report = move_uploaded_file($_FILES['img']['tmp_name'], '../assets/img/'.$fname);
			if($move_report){
				$data .=", upload_report = '$fname_report' ";
			}
		}
		if(empty($id)){
			$save = $this->db->query("INSERT INTO reports set ".$data);
		}else{
			$save = $this->db->query("UPDATE reports set ".$data." where id=".$id);
		}
		if($save)
			return 1;
	}
	function delete_report(){
		extract($_POST);
		$delete = $this->db->query("DELETE FROM reports where id = ".$id);
		if($delete)
			return 1;
	}
	...
	function save_flight(){
		...
		$data .= ", take_off_time = '$take_off_time' ";
